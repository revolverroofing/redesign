...
- **Two Server Actions, two delivery functions.** Consumer form posts to `submitLead` → `deliverLead`; GC bid form posts to `submitBid` → `deliverBid`. They are separate so estimating never has to filter through residential intake. Both POST JSON to Zapier when `LEAD_ZAPIER_HOOK_URL` / `BID_ZAPIER_HOOK_URL` env vars are set (Zap → Webhooks by Zapier → Catch Hook), otherwise fall back to logging on the server console so dev installs aren't silent. Set the env vars in the production deployment (Vercel / Netlify dashboard, or `.env.local` for local dev — `.env*` is gitignored).
